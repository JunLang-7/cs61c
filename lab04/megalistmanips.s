...
map:
    ...
    # wrong 1: add for the address but lw for value
    lw t1, 0(s0)        # load the address of the array of current node into t1
    lw t2, 4(s0)        # load the size of the node's array into t2
    addi t1, t1, -4
mapLoop:

    # wrong 2: incorrect increment for array
    ...
    # wrong 4: la for the address but lw for value
    lw a0, 8(s0)        # load the address of the next node into a0
    
    # wrong 5: lw for the value but add for address
